...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--fixture-dir",
        default=None,
        help="Read checks, run metadata, and logs from a local fixture directory instead of calling gh.",
    )
...
def main() -> int:
    ...
    if fixture_dir is not None:
        if not fixture_dir.is_dir():
            ...
            return 1
        pr_value = args.pr or "fixture"
        checks = load_fixture_checks(fixture_dir)
    else:
...
def load_fixture_json(path: Path) -> Any:
    try:
        ...
        print(f"Error: unable to parse fixture JSON: {path}", file=sys.stderr)
        return None


def load_fixture_checks(fixture_dir: Path) -> list[dict[str, Any]] | None:
    path = fixture_dir / "checks.json"
    data = load_fixture_json(path)
    if data is None:
        return None
    if not isinstance(data, list):
        print(f"Error: unexpected checks fixture shape in {path}", file=sys.stderr)
        return None
    return data
...
def analyze_check(
    ...
    metadata = fetch_run_metadata(run_id, repo_root, fixture_dir=fixture_dir)
...
def fetch_run_metadata(run_id: str, repo_root: Path | None, *, fixture_dir: Path | None = None) -> dict[str, Any] | None:
    if fixture_dir is not None:
        path = fixture_dir / f"run_{run_id}.json"
        if not path.exists():
            return None
        data = load_fixture_json(path)
...
def fetch_check_log(
    ...
    if fixture_dir is not None:
        return fetch_fixture_log(run_id=run_id, job_id=job_id, fixture_dir=fixture_dir)
    if repo_root is None:
        return "", "Error: repository root is unavailable.", "error"
...
def fetch_fixture_log(run_id: str, job_id: str | None, fixture_dir: Path) -> tuple[str, str, str]:
    candidate_paths: list[Path] = []
    if job_id:
        candidate_paths.extend([
            fixture_dir / f"job_{job_id}.log",
            fixture_dir / f"job_{job_id}.txt",
        ])
    candidate_paths.extend([
        fixture_dir / f"run_{run_id}.log",
        fixture_dir / f"run_{run_id}.txt",
    ])
    for path in candidate_paths:
        if not path.exists():
            continue
        text = path.read_text(encoding="utf-8", errors="replace")
        if is_log_pending_message(text):
            return "", text.strip(), "pending"
        return text, "", "ok"
    return "", f"Fixture log not found for run {run_id}.", "error"
