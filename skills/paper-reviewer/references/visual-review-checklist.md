# Visual Review Checklist

Use this checklist during the PDF page-image pass.

## Layout and typography
- Is the visual hierarchy clear from title through captions and references?
- Are body text, headings, captions, footnotes, and equations consistently sized and spaced?
- Are there crowded pages, large dead zones, awkward page breaks, widows/orphans, overflow, or clipped content?
- Are figures, tables, and equations placed near their first discussion in the text?

## Figures, tables, and charts
- Are figures legible at normal page scale, including panel labels, legends, axis labels, and annotations?
- Do charts use clear encodings, units, baselines, uncertainty markers, and readable color choices?
- Are tables readable without zooming excessively, with sane spacing and numeric alignment?
- Do multi-panel figures feel balanced, ordered, and easy to scan?

## Captions and callouts
- Does each caption describe what is visible and state the intended takeaway?
- Do figure/table references in the text point to the correct visual element?
- Are there mismatches between captions, panel labels, legends, or plotted content?

## Review expectations
- Prioritize the highest-impact visual issues instead of listing every cosmetic nit.
- Use page numbers when possible.
- If the image resolution is insufficient, say so and lower confidence instead of guessing.
