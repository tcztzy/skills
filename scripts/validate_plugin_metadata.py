...
def validate_codex_metadata(repo_root: Path) -> list[str]:
    ...
    if not isinstance(plugin_name, str) or not plugin_name:
        errors.append(f"{repo_label(plugin_manifest_path)} is missing a string 'name'.")
        return errors

    plugin_root = plugin_manifest_path.parent.parent
    skills_rel = plugin_manifest.get("skills")
    if not isinstance(skills_rel, str) or not skills_rel:
        errors.append(
            f"{repo_label(plugin_manifest_path)} is missing a string 'skills' path."
        )
    else:
        skills_path = plugin_root / skills_rel.removeprefix("./")
        if not skills_path.exists():
            errors.append(
                f"{repo_label(plugin_manifest_path)} points to a missing skills path: "
                f"{skills_rel}"
            )
